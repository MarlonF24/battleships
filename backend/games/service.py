...
async def join_game(player: Player, game: Game, session: AsyncSession) -> GamePhase:
    ...
    if num_current_players == 0:
        logger.critical(f"Player ID {player.id} tried to join game ID {game.id} which has no players. Such a game should not exist. Games must be created with one player.")
    ...
    if not player in current_players:
        if num_current_players == 1:
            try:
                ...
                logger.critical(f"IntegrityError: Player ID {player.id} could not join game ID {game.id}. Possibly because the game creator was saved in slot 2. This should never happen. Error: {e}")
                raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="Unable to join game.")

        else:
            logger.warning(f"Player ID {player.id} attempted to join full game ID {game.id}")
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Unable to join full game.")
    
    if game.phase == GamePhase.COMPLETED:
        logger.warning(f"Player ID {player.id} attempted to join completed game ID {game.id}")
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Unable to join completed game.")
